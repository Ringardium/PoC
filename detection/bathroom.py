...
def check_trigger(
    ...
    조건 1: bbox 높이가 최근 평균 대비 일정 비율 이상 감소 (웅크림)
    조건 2: 이동량이 적음 (정지 상태)
    ...
    Args:
        ...
        height_drop_ratio: 높이 감소 비율 임계값 (0.25 = 25% 이상 감소)
    ...
    # 전반부 평균 높이 vs 후반부 평균 높이
    half = len(heights) // 2
    early_height = np.mean(heights[:half])
    recent_height = np.mean(heights[half:])

    if early_height < 1e-6:
        return False

    # 조건 1: 높이 감소율
    drop_ratio = (early_height - recent_height) / early_height
    if drop_ratio < height_drop_ratio:
        return False
...
def detect_bathroom(
    ...
    Args:
        ...
        height_drop_ratio: 높이 감소 비율 임계값
        displacement_threshold: 이동량 임계값
