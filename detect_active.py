import numpy as np


def detect_active(
    active_coor,
    active_threshold,
    active_frames,
):
    """Detect actively moving (running/playing) pets.

    Inverse of inert detection: flags pets whose cumulative displacement
    over the last ``active_frames`` exceeds ``active_threshold``.

    Args:
        active_coor: dict mapping track_id -> deque of [x, y] centres.
        active_threshold: minimum cumulative displacement to be considered active.
        active_frames: minimum number of coordinate history frames required.

    Returns:
        List of track_ids that are currently active.
    """
    active_id = []

    for id in active_coor.keys():
        coor = np.array(active_coor[id])

        if len(coor) < active_frames:
            continue

        distances = np.sum((coor[1:] - coor[:-1]) ** 2, axis=1)
        displacement = np.sum(distances)

        if displacement > active_threshold:
            active_id.append(id)

    return active_id
